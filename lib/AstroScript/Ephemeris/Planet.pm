package AstroScript::Ephemeris::Planet;
use strict;
use warnings;

use Readonly;
use Math::Trig qw/:pi rad2deg/;
use AstroScript::MathUtils qw/frac ARCS spherical rectangular/;
...
sub _geocentric {
    ...
    ,    # ecliptic heliocentric coordinates of the planet
      ...
      { x => $x, y => $y,
        z => $z }    # ecliptic geoocentric coordinates of the planet
}
...
AstroScript::Ephemeris modules. Subclasses must implement B<heliocentric> method.

=head1 SUBROUTINES/METHODS

=head2 $planet = AstroScript::Ephemeris::Planet->new( $id )
...
=item B<$sun> — ecliptic geocentric coordinates of the Sun (hashref with B<'x'>, B<'y'>, B<'z'> keys)
